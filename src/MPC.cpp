...
double rad2deg_(double x) { return x * 180 / M_PI; }
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) 
  {
    ...
    for (t = 1; t < N; t++)
    {
      ...
      // Take actuation into consideration at time (t) only
      AD<double> delta0 = vars[delta_start + t - 1];
      AD<double> a0     = vars[a_start + t - 1];

      // Initial Polynomial @ X0
      ...
      AD<double> psi_des0 = CppAD::atan(coeffs[1] + 
                                        2 * coeffs[2] * x0 + 
                                        3 * coeffs[3] * CppAD::pow(x0,2)); // Differentiation of the Initial Polynomial @ X0

      
      // Apply having constaints = 0
      ...
      fg[epsi_start + t + 1] = epsi1 - ((psi0 - psi_des0) - v0/Lf * delta0 * dt);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set States Boundaries
  // X, Y, psi, v, cte, epsi Lower and Upper bounds approx = 0 
  ...
  // Set Actuators Boundaries
  // Steering Angle Lower and Upper bounds from -25 to +25
  for (i = delta_start; i < a_start; i++)
  {
      vars_lowerbound[i] = deg2rad_(-25.0);
      vars_upperbound[i] = deg2rad_(25.0);
  }

  // Accelertaion and Deceleration Lower and Upper bounds from -1 to +1
